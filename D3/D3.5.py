...
for i in range(0,len(dataNew)):
    ...
    for j in dataNew[i]:
        for k in range(1,j[1]+1):
            ...
            if tuple(currentPos) in distDict:
                if distDict[i][tuple(currentPos)] > distTravelled:
                    distDict[i][tuple(currentPos)] = distTravelled
            else:     
                distDict[i][tuple(currentPos)] = distTravelled
results = visitedCoords[0].intersection(visitedCoords[1])
distances=[]
for i in results:
    distances.append(distDict[0][i] + distDict[1][i])
minTravelled = min(distances)
print(minTravelled)